...
import { isOrganization } from "../types/author";

export async function showPkgInfo(pkg: string) {
    const manifest = await KPI.manifest(pkg);
    const author = await KPI.authorManifest(manifest.author_id);
...
export async function showUserInfo(usr: string) {
    const author = await KPI.authorManifest(usr);
    ...
    const website = author.website
        ...
        : "(No website provided)";
    const twitter = author.socials.twitter
        ? `${"Twitter"} ${konsole.clr("lightblue", `@${author.socials.twitter}`)}`
        : "";
    const github = author.socials.github
        ? `${"GitHub"} ${konsole.clr("lightblue", `@${author.socials.github}`)}`
        : "";
    konsole.adv([name, email, website].filter(Boolean).join(" · "));
    konsole.adv([twitter, github].filter(Boolean).join(" · "));
