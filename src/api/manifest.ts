...
export function parseKps(scope: KONBINI_PKG_SCOPE): {
    ...
    if (!splitted[0]) throw `Invalid KPS (no prefix): ${scope}`;
    if (!splitted[1]) throw `Invalid KPS (no suffix): ${scope}`;
...
export function getCurrentPlatformKps(
    ...
    if (process.platform === "linux") {
        if (process.arch === "x64") return platforms.linux64;
        else if (process.arch === "arm64") return platforms.linuxARM;
        throw "Impossible error - Konbini is running from an unsupported Linux platform.";
    } else if (process.platform === "darwin") {
        if (process.arch === "x64") return platforms.mac64;
        else if (process.arch === "arm64") return platforms.macARM;
        throw "Impossible error - Konbini is running from an unsupported Apple macintoshOS platform.";
    } else if (process.platform === "win32") {
        if (process.arch === "x64") return platforms.win64;
...
export function getCurrentPlatformShaKey(): keyof KONBINI_HASH_FILE {
